...
describe('splitPolygonAtIndices', () => {
  describe('simple rectangle splits', () => {
    it('should split a rectangle vertically into two equal parts', () => {
      ...
      const cutStart = vec2.fromValues(50, 100) // On edge from (100,100) to (0,100)
      const cutEnd = vec2.fromValues(50, 0) // On edge from (0,0) to (100,0)
    ...
    it('should split a rectangle horizontally into two equal parts', () => {
      ...
      const cutStart = vec2.fromValues(100, 50) // On edge from (100,0) to (100,100)
      const cutEnd = vec2.fromValues(0, 50) // On edge from (0,100) to (0,0)
  ...
  describe('L-shape split', () => {
    it('should split L-shape into two rectangles', () => {
      ...
      const cutStart = vec2.fromValues(50, 50) // On edge index 2, at the starting vertex
      const cutEnd = vec2.fromValues(50, 100) // On edge from points[4] to points[5]
  ...
  describe('edge cases', () => {
    it('should handle cut points that coincide with vertices', () => {
      ...
      const cutStart = vec2.fromValues(100, 100) // Exactly at point index 2
      const cutEnd = vec2.fromValues(0, 0) // Exactly at point index 0
