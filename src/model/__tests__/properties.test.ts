          ...
          for (const op of operations) {
            try {
              ...
              if (op.shouldRemove && addedWalls.length > 1) {
                ...
                if (wallToRemove !== null && wallToRemove !== undefined && wallToRemove !== '') {
      ...
      fc.assert(fc.property(
        ...
        (points) => {
          ...
          for (let i = 1; i < connectionPoints.length; i++) {
            ...
            if (centerConn !== undefined && !centerConn.connectedWallIds.includes(wall.id)) {
              centerConn.connectedWallIds.push(wall.id)
            }
            if (endConn !== undefined && !endConn.connectedWallIds.includes(wall.id)) {
              endConn.connectedWallIds.push(wall.id)
            }
          }

          const centerConn = building.connectionPoints.get(centerPoint.id)
          if (centerConn !== undefined) {
      ...
      fc.assert(fc.property(
        ...
        (points) => {
          ...
          if (walls.length > 0) {
            ...
            if ((startPoint != null) && !building.connectionPoints.has(startPoint.id)) {
